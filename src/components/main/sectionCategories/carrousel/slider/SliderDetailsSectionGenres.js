...
export default function SliderDetailsSectionGenres({ genres, large = false }){
  ...
  const newGenresMap = newGenres.map(elem => (
    <div key={elem.id} className='flex flex-row items-center'>
      <BsDot className={`text-gray-tones-100 ${large ? 'text-2xl' : 'text-xs'}`} />
      <p className={`m-0 p-0 ${large ? 'text-tiny' : 'text-s'} text-gray-tones-10 font-semibold leading-none`}>{elem.name}</p>
    </div>
  ))

  return (
    <div className={`flex gap-0 ${large ? 'mt-4 w-80' : 'mt-1.5'}`}>
      {newGenresMap}
    </div>
  )
}